...
/* Result: 0: simulation terminated successfully  */
/*         1 : simulation did not terminate successfully; output file written upto termination time point */
/*         2: simulation did not terminate successfully; no output file written */
#endif
...
LIBDS_API int DymosimRun2(struct DymosimSimulator*, double tStop); //caling run, but using end step time to avoid roundof
...
LIBDS_API int DymosimRunToEvent2(struct DymosimSimulator* sim, double* p_tStop, double tStep /* not used yet*/, int* hadEvent /* may be null*/, int allowEarlyStop);
/* Run a simulation.*/
...
static DYN_UNUSED_ATTR int DymosimIsParameter(struct DymosimSimulator*sim,struct DymosimVariableHandle*h) {
	return h && h->category==dsParameter;
}
static DYN_UNUSED_ATTR int DymosimIsInput(struct DymosimSimulator*sim,struct DymosimVariableHandle*h) {
	return h && h->category==dsInput;
}
static DYN_UNUSED_ATTR int DymosimIsOutput(struct DymosimSimulator*sim,struct DymosimVariableHandle*h) {
...
LIBDS_API int DymosimGetReal(struct DymosimSimulator*dymosimData, const unsigned int vr[], size_t nvr, double val[]);
/* Returns current value of variables */
/* More closely resembles the FMI interface */
...
LIBDS_API int DymosimSetReal(struct DymosimSimulator*dymosimData, const unsigned int vr[], size_t nvr, const double value[]);
/* Sets value or variable. Returns 0 if ok. -1 for computation error and -2 for illegal index */
/* More closely resembles the FMI interface */
...
LIBDS_API int DymosimInitializeLicensing( struct DymosimSimulator*sim, const char *key );
...
LIBDS_API int DymosimGetFMUState(struct DymosimSimulator*sim, void ** fmuState, int currentState);

LIBDS_API int DymosimSetFMUState(struct DymosimSimulator*sim, void * fmuState, int* currentState);
...
LIBDS_API int DymosimTerminate(struct DymosimSimulator* sim);
/*Splits up terminate part from TerminateAndFree, should curently only be used for FMU*/
