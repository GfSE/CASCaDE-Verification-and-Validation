...
/* DS: renamed from TRY_AGAIN to avoid clash with winsock2.h on MinGW */
...
SUNDIALS_EXPORT void cvProcessError(CVodeMem cv_mem, int error_code, const char *module,
...
SUNDIALS_EXPORT int cvNlsInit(CVodeMem cv_mem);

/* Projection functions */

SUNDIALS_EXPORT int cvDoProjection(CVodeMem cv_mem, int *nflagPtr, realtype saved_t,
                   int *npfPtr);
SUNDIALS_EXPORT int cvProjInit(CVodeProjMem proj_mem);
SUNDIALS_EXPORT int cvProjFree(CVodeProjMem *proj_mem);
