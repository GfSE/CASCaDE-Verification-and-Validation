...
SUNDIALS_EXPORT void *CVodeCreate(int lmm, SUNContext sunctx)
{
  ...
  cv_mem->cv_next_time_event = DBL_MAX;
          ...
          realtype *tret, int itask)
{
  ...
  if (cv_mem->cv_nst > 0) {
    ...
    if (cv_mem->cv_nrtfn > 0) {
      ...
      if ( SUNRabs(cv_mem->cv_tn - cv_mem->cv_tretlast) > troundoff ) {
        ...
        if (retval == CV_SUCCESS) {     /* no root found */
          if (itask == CV_ONE_STEP && cv_mem->cv_next_time_event < cv_mem->cv_tn) {
              cv_mem->cv_thi = cv_mem->cv_next_time_event;
              (void) CVodeGetDky(cv_mem, cv_mem->cv_thi, 0, cv_mem->cv_y);
              cv_mem->cv_irfnd = 1;
              cv_mem->cv_tretlast = *tret = cv_mem->cv_thi;
              return(CV_TIME_ROOT_RETURN);
          }
          cv_mem->cv_irfnd = 0;
    ...
    } else if (itask == CV_ONE_STEP && cv_mem->cv_next_time_event < cv_mem->cv_tn) {
        cv_mem->cv_thi = cv_mem->cv_next_time_event;
        (void) CVodeGetDky(cv_mem, cv_mem->cv_thi, 0, cv_mem->cv_y);
        cv_mem->cv_irfnd = 1;
        cv_mem->cv_tretlast = *tret = cv_mem->cv_thi;
  ...
  for(;;) {
    ...
    if (itask == CV_ONE_STEP && cv_mem->cv_next_time_event < cv_mem->cv_tn) {
        ...
        (void) CVodeGetDky(cv_mem, cv_mem->cv_thi, 0, cv_mem->cv_y);
        cv_mem->cv_irfnd = 1;
        istate = CV_TIME_ROOT_RETURN;
        cv_mem->cv_tretlast = *tret = cv_mem->cv_thi;
        break;
    }

	if (cv_mem->cv_stepgfun && (cv_mem->cv_stepgfun)(cv_mem->cv_user_data)) {
...
SUNDIALS_EXPORT void cvRescale(CVodeMem cv_mem)
{
  int j;
...
#ifdef GODESS
typedef realtype doublereal;
extern int GetAdditionalFlags(int flag_num);
extern void updateErrCount2(doublereal time, doublereal step_size, int order, int rejected,
	int neq, const doublereal*wt, const doublereal*e, doublereal err, doublereal mult, int invW, 
	doublereal start_time);
#endif /* GODESS */
...
#ifdef GODESS
  if (GetAdditionalFlags(19))
    updateErrCount2(cv_mem->cv_tn, cv_mem->cv_h, 1, 0, 0, 0, 0, 1.0e-10, 1.0, 0, 0.0); 
    /* Not sure if we should actually modify the h-input according to below. Needs investigations. */
#endif /* GODESS */
...
SUNDIALS_EXPORT void cvRestore(CVodeMem cv_mem, realtype saved_t)
{
  int j, k;
...
static int cvRcheck3(CVodeMem cv_mem)
{
  ...
  if (cv_mem->cv_taskc == CV_ONE_STEP) {
    if (cv_mem->cv_next_time_event < cv_mem->cv_tn) {
        cv_mem->cv_thi = cv_mem->cv_next_time_event;
        (void) CVodeGetDky(cv_mem, cv_mem->cv_thi, 0, cv_mem->cv_y);
    } else {
        cv_mem->cv_thi = cv_mem->cv_tn;
