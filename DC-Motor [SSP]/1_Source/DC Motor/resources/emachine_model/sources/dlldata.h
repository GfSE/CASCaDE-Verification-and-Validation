...
struct SimulatorFunctions {
	...
	struct DymosimSimulator* (*newf2)(const char*,const char*,const char*,void*,void*,void(*Err)(enum Dymosim_Severity,const char*),int, const char*, int, int);
	int (*run)(struct DymosimSimulator*,double);
	int (*run2)(struct DymosimSimulator*, double);
	...
	int (*getFMUState)(struct DymosimSimulator*, void**, int);
	int (*setFMUState)(struct DymosimSimulator*, void*, int*);
	...
	int (*getReal)(struct DymosimSimulator*dymosimData, const unsigned int vr[], size_t nvr, double value[]);
	int (*setReal)(struct DymosimSimulator*dymosimData, const unsigned int vr[], size_t nvr, const double value[]);
	int (*terminate)(struct DymosimSimulator* sim);
	void (*setNominal)(struct DymosimSimulator*, double x, int ix);
	int (*runToEvent2)(struct DymosimSimulator*, double*p_tStop, double tStep /* not used yet*/, int*hadEvent /* may be null*/, int allowEarlyStop);
	int (*setupIntermediate)(struct DymosimSimulator*, void (*intermediateCallBack) (
		...
		double* earlyReturnTime), void* instanceEnvironment, const unsigned int vr[], size_t nvr);
};
#include "dsblock.h"
...
struct DymosimSimulator { 
	...
	char* pNameInternal; /*storage for panme*/
	...
	struct {
		...
		integer ncom, imeth, nt, evuord, pdebug, fmax, ordmax, eviter, memkey, 
			...
			debug, fexcep, shared, enable_steady_state_termination, fail_if_steady_state_not_reached,
			is_steady_state_simulation, steady_state_termination_strict, steady_state_ignore_low_order_states;
	} aadsm1_;
	...
	struct {
		integer sgopt, iasdb, ias1, steady_state_reached;
	} aadsm9_;
	...
	struct SteadyStateUtils* steady_state_utils;
	...
	struct  {
		...
		int max_run_time_reached;
		double one_simulation_total_timer_start;
	} dsRes;
...
static DYN_UNUSED_ATTR void dymosimUnSetThreadData(struct DymosimSimulator*data) {
...
#endif
static DYN_UNUSED_ATTR struct DymosimSimulator* dymosimGetThreadData() {
	return dataNoDllPtr ? dataNoDllPtr : &dataNoDll;
}
static DYN_UNUSED_ATTR void dymosimSetThreadData(struct DymosimSimulator*data) {
